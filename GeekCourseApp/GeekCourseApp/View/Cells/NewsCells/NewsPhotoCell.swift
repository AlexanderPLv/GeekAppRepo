...
class NewsPhotoCell: UITableViewCell, ReusableView {
    ...
    func set(_ news: News) {
        ...
        if let imageUrl = news.newsImageUrl {
            newsImage.loadImage(urlString: imageUrl)
        } else {
            newsImage.image = UIImage(named: "imgP")
        }
